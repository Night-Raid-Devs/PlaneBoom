...
namespace Turrets
{
   public class TurretRotation : MonoBehaviour
   {
      ...
      {
         if (aiming == false)
            aimPoint = transform.TransformPoint(Vector3.forward * 900.0f);
      }
      ...
      private void RotateBarrels()
      {
         ...
         if (turretBase != null && turretBarrels != null)
         {
            ...
            // Create new rotation towards the target in local space.
            Quaternion rotationGoal = Quaternion.LookRotation(clampedLocalVec2Target);
            Quaternion newRotation = Quaternion.RotateTowards(turretBarrels.localRotation, rotationGoal, 10.0f * turnRate * Time.deltaTime);
      ...
      private bool RotateToIdle()
      {
         ...
         {
            Quaternion newRotation = Quaternion.RotateTowards(turretBarrels.localRotation, Quaternion.identity, 10.0f * turnRate * Time.deltaTime);
            turretBarrels.localRotation = newRotation;
      ...
      {
         if (turretBarrels != null)
            Debug.DrawRay(turretBarrels.position, turretBarrels.forward * 900.0f);
         else if (turretBase != null)
            Debug.DrawRay(turretBase.position, turretBase.forward * 900.0f);
      }
   }
}
